...
namespace svd{
    ...
    double p[USER_NUM+1][K_NUM+1] = {0};   //ÓÃÓÚ´æ´¢ÓÃ»§µÄÊôÐÔÃèÊöp   user character Matrix
    double puTemp[USER_NUM+1][K_NUM+1] = {0};   //ÓÃÓÚ´æ´¢ÓÃ»§µÄÊôÐÔÃèÊöp user complete character
    double q[ITEM_NUM+1][K_NUM+1] = {0};   //ÓÃÓÚitemµÄÊôÐÔÃèÊöq       item character Matrix
               ...
               int maxStep=60,double slowRate=1,bool isUpdateBias=true)
    {
        ...
        for(int step = 0; step < maxStep; ++step){  //only iterate maxStep times
            ...
            for( u = 1; u < USER_NUM+1; ++u) {   //process every user (Ñ­»·´¦ÀíÃ¿Ò»¸öÓÃ»§)
                ...
                for( k=1; k<K_NUM+1; ++k) {
                    double sumy = 0.0;
                    for(i=0; i < RuNum; ++i) {//  process every item rated by user u(Ñ­»·´¦Àíu´ò·Ö¹ýµÄÃ¿Ò»¸öitem)
                ...
                for(j=0; j < RuNum; ++j) {// process every item rated by user u(Ñ­»·´¦Àíu´ò·Ö¹ýµÄÃ¿Ò»¸öitem)
